...
#C5.0 is a classifier, so you'be teaching it how to classify the different species of irises using the training data.
library(C50)
...
#this model uses decision tree to vividly represent members of each node.each path is taken when a condition is met.
#For instance, node 2 falls where petal length is less or equal to 1.9 and node 3 where petal lenth is greater than 1.9. In addition, node 4 holds that petal length should be less than or equal to 4.9 while for node 5, petal length should be greater than 4.9 
p<-predict(dtModel, testData, type="class")
#TASK: What percent of cases were correctly classified?
sum( p == trainData$Species ) / length( p )
 0.8888889=88.89%
